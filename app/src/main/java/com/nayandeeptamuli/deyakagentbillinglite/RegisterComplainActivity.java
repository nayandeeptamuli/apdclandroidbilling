...
public class RegisterComplainActivity extends ActionBarActivity implements OnItemSelectedListener {
    ...
    class ExecuteCheckPrepostTask extends AsyncTask<String, Integer, String> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            this.progressDialog = ProgressDialog.show(RegisterComplainActivity.this, "Fetching User details...", "Please wait...", false, true);
        }

        protected String doInBackground(String... params) {
            return RegisterComplainActivity.this.checkPrepost(params);
        }

        protected void onPostExecute(String result) {
            this.progressDialog.dismiss();
            if (!result.startsWith("PP")) {
                ((TextView) RegisterComplainActivity.this.findViewById(R.id.message)).setText("This facility is currently available for R-APDRP post paid consumers only. For any complain, please call our toll free number 1912");
                ((TextView) RegisterComplainActivity.this.findViewById(R.id.consumerid_lbl)).setVisibility(8);
                ((TextView) RegisterComplainActivity.this.findViewById(R.id.consumerId)).setVisibility(8);
                ((Spinner) RegisterComplainActivity.this.findViewById(R.id.main_category)).setVisibility(8);
                ((Spinner) RegisterComplainActivity.this.findViewById(R.id.sub_category)).setVisibility(8);
                ((TextView) RegisterComplainActivity.this.findViewById(R.id.prbdesc_lbl)).setVisibility(8);
                ((EditText) RegisterComplainActivity.this.findViewById(R.id.prob_desc)).setVisibility(8);
                RegisterComplainActivity.this.findViewById(R.id.btn_complain).setVisibility(8);
            } else if (result.indexOf("HTML") != -1) {
                ((TextView) RegisterComplainActivity.this.findViewById(R.id.message)).setText("Server busy.Please try after some time.");
                ((TextView) RegisterComplainActivity.this.findViewById(R.id.consumerid_lbl)).setVisibility(8);
                ((TextView) RegisterComplainActivity.this.findViewById(R.id.consumerId)).setVisibility(8);
                ((Spinner) RegisterComplainActivity.this.findViewById(R.id.main_category)).setVisibility(8);
                ((Spinner) RegisterComplainActivity.this.findViewById(R.id.sub_category)).setVisibility(8);
    ...
    class RaiseComplainClass extends AsyncTask<String, Integer, String> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            this.progressDialog = ProgressDialog.show(RegisterComplainActivity.this, "Creating docket...", "Please wait...", false, true);
        }

        protected String doInBackground(String... params) {
            return RegisterComplainActivity.this.postComplaint(params);
        }
        ...
        protected void onPostExecute(String result) {
            ...
            if (docketResult.matches("\\d+")) {
                ((TextView) RegisterComplainActivity.this.findViewById(R.id.message)).setText("Your complaint has been raised successfully.Please note the Docket Number : " + result.substring(0, result.indexOf("<")) + " for future reference.");
                ((TextView) RegisterComplainActivity.this.findViewById(R.id.consumerid_lbl)).setVisibility(8);
                ((TextView) RegisterComplainActivity.this.findViewById(R.id.consumerId)).setVisibility(8);
                ((Spinner) RegisterComplainActivity.this.findViewById(R.id.main_category)).setVisibility(8);
                ((Spinner) RegisterComplainActivity.this.findViewById(R.id.sub_category)).setVisibility(8);
            ...
            ((TextView) RegisterComplainActivity.this.findViewById(R.id.message)).setText("Your complaint can not be raised as " + docketResult + ". Please try again.");
            ((TextView) RegisterComplainActivity.this.findViewById(R.id.consumerid_lbl)).setVisibility(8);
            ((TextView) RegisterComplainActivity.this.findViewById(R.id.consumerId)).setVisibility(8);
            ((Spinner) RegisterComplainActivity.this.findViewById(R.id.main_category)).setVisibility(8);
            ((Spinner) RegisterComplainActivity.this.findViewById(R.id.sub_category)).setVisibility(8);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView((int) R.layout.activity_register_complain);
        if (getIntent().getExtras().getString("ConsumerId").toString().trim().length() == 12) {
            ((TextView) findViewById(R.id.message)).setText("This facility is currently available for R-APDRP post paid consumers only. For any complain, please call our toll free number 1912");
        ...
        ((Button) findViewById(R.id.btn_complain)).setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                RegisterComplainActivity.this.raiseCompaint();
            }
        });
    }
    ...
    public String postComplaint(String[] values) {
        ...
        try {
            ...
            String reqXML = "";
            StringEntity se = new StringEntity("<?xml version=\"1.0\" encoding=\"utf-8\"?><SOAP-ENV:Envelope xmlns:SOAP-ENV=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:xs=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"><SOAP-ENV:Body><yq1:raiseComplain xmlns:yq1=\""><conId>" + values[0] + "</conId>" + "<mainCategory>" + values[1] + "</mainCategory>" + "<subCategory>" + values[2] + "</subCategory>" + "<probDesc>" + values[3] + "</probDesc>" + "</yq1:raiseComplain>" + "</SOAP-ENV:Body>" + "</SOAP-ENV:Envelope>");
    ...
    public String checkPrepost(String[] params) {
        ...
        try {
            ...
            httpPost.setHeader("Content-type", "text/xml; charset=utf-8");
            String reqXML = "";
            StringEntity se = new StringEntity("<?xml version=\"1.0\" encoding=\"UTF-8\"?><SOAP-ENV:Envelope xmlns:SOAP-ENV=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:ns1=\""><SOAP-ENV:Body><ns1:returnConsumerDetails><consumerId>" + params[0] + "</consumerId>" + "</ns1:returnConsumerDetails>" + " </SOAP-ENV:Body>" + "</SOAP-ENV:Envelope>");
            se.setContentEncoding(new BasicHeader("Content-Type", "application/json"));
            httpPost.setEntity(se);
            HttpResponse httpResponse = httpClient.execute(httpPost);
            HttpEntity httpEntity = httpResponse.getEntity();
